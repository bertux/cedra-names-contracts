#[test_only]
module cedra_names::test_helper {
    ...
    public fun fq_domain_name(): String {
        string::utf8(b"test.ced")
    }

    public fun fq_subdomain_name(): String {
        string::utf8(b"sub.test.ced")
    }
    ...
    public fun register_name(
        ...
        let user_balance_before = coin::balance<CedraCoin>(user_addr);
        let user_reverse_lookup_before = domains::get_reverse_lookup(user_addr);
        let maybe_target_address =
            domains::name_resolved_address(subdomain_name, domain_name);
        ...
        let is_expired_before =
            domains::name_is_registered(subdomain_name, domain_name)
                && domains::name_is_expired(subdomain_name, domain_name);
        let register_name_event_v1_event_count_before =
            domains::get_register_name_event_v1_count();
        let set_name_address_event_v1_event_count_before =
            domains::get_set_name_address_event_v1_count();
        let set_reverse_lookup_event_v1_event_count_before =
        ...
        if (option::is_none(&subdomain_name)) {
            if (vector::length(&signature) == 0) {
                ...
                domains::register_domain_with_signature(
                    user, domain_name, years, signature
                );
            }
        } else {
        ...
        let (is_owner, _) =
            domains::is_owner_of_name(user_addr, subdomain_name, domain_name);
        assert!(is_owner, 3);
        let (is_owner, token_id) =
            domains::is_token_owner(user_addr, subdomain_name, domain_name);
        let (tdi_creator, tdi_collection, tdi_name, tdi_property_version) =
        ...
        test_utils::print_actual_expected(
            b"tdi_name: ",
            tdi_name,
            expected_fq_domain_name,
            false
        );
        assert!(tdi_name == expected_fq_domain_name, 6);
        test_utils::print_actual_expected(
            b"tdi_property_version: ",
            tdi_property_version,
        ...
        let user_balance_after = coin::balance<CedraCoin>(user_addr);
        if (is_subdomain) {
            // If it's a subdomain, we only charge a nomincal fee
            expected_user_balance_after =
                user_balance_before
                    - price_model::price_for_subdomain_v1(registration_duration_secs);
        } else {
            let domain_price =
                price_model::price_for_domain_v1(string::length(&domain_name), years);
            assert!(
                domain_price / config::octas() == 40,
                domain_price / config::octas()
            );
        ...
        assert!(
            user_balance_after == expected_user_balance_after,
            expected_user_balance_after
        );

        // Ensure the name was registered correctly, with an expiration timestamp one year in the future
        let (property_version, expiration_time_sec, target_address) =
            domains::get_name_record_v1_props_for_name(subdomain_name, domain_name);
        assert!(
            time_helper::seconds_to_days(expiration_time_sec - timestamp::now_seconds())
                == 365,
            10
        );
        ...
        let token_data_id =
            token_helper::build_tokendata_id(
                token_helper::get_token_signer_address(), subdomain_name, domain_name
            );
        let (creator, collection_name, token_name) =
            token::get_token_data_id_fields(&token_data_id);
        assert!(creator == token_helper::get_token_signer_address(), 20);
        assert!(collection_name == string::utf8(b"Cedra Names V1"), 21);
        assert!(token_name == token_name, 22);

        // Assert events have been correctly emmitted
        let register_name_event_v1_num_emitted =
            domains::get_register_name_event_v1_count()
                - register_name_event_v1_event_count_before;
        let set_name_address_event_v1_num_emitted =
            domains::get_set_name_address_event_v1_count()
                - set_name_address_event_v1_event_count_before;
        ...
        assert!(
            register_name_event_v1_num_emitted == 1, register_name_event_v1_num_emitted
        );
        ...
        if (option::is_none(&user_reverse_lookup_before)) {
            ...
            if (option::is_some(&maybe_reverse_lookup_after)) {
                ...
                assert!(
                    *reverse_lookup_after
                        == domains::create_name_record_key_v1(
            ...
            if (option::is_some(&name_reverse_lookup_before) && is_expired_before) {
                assert!(
                    set_reverse_lookup_event_v1_num_emitted == 2,
                    set_reverse_lookup_event_v1_num_emitted
                );
            } else {
                assert!(
                    set_reverse_lookup_event_v1_num_emitted == 1,
                    set_reverse_lookup_event_v1_num_emitted
                );
            }
        } else {
            ...
            if (option::is_some(&name_reverse_lookup_before)
                ...
                && *option::borrow(&name_reverse_lookup_before)
                    == *option::borrow(&user_reverse_lookup_before)
                && is_expired_before) {
                assert!(
                    set_reverse_lookup_event_v1_num_emitted == 2,
                    set_reverse_lookup_event_v1_num_emitted
                );
            } else if (option::is_some(&name_reverse_lookup_before)
                && is_expired_before) {
                // If we are registering over a name that is already registered but expired and was a primary name,
                // that name should be removed from being a primary name.
                assert!(
                ...
                assert!(
                    set_reverse_lookup_event_v1_num_emitted == 0,
                    set_reverse_lookup_event_v1_num_emitted
                );
            }
        };
        ...
        if (is_subdomain) {
            if (option::is_none(&user_reverse_lookup_before)) {
                // Should automatically point to the users address
                test_utils::print_actual_expected(
                    b"set_name_address_event_v1_num_emitted: ",
                    set_name_address_event_v1_num_emitted,
                    1,
                    false
                );
                assert!(
                    set_name_address_event_v1_num_emitted == 1,
                    set_name_address_event_v1_num_emitted
                );
            } else {
                // We haven't set a target address yet!
                ...
                assert!(
                    set_name_address_event_v1_num_emitted == 0,
                    set_name_address_event_v1_num_emitted
                );
            }
        } else {
    ...
    public fun set_name_address(
        ...
        assert!(
            register_name_event_v1_num_emitted == 0, register_name_event_v1_num_emitted
        );
        ...
        if (option::is_some(&maybe_reverse_lookup_before)) {
            ...
            if (maybe_reverse_subdomain == subdomain_name
                && reverse_domain == domain_name
                && signer::address_of(user) != expected_target_address) {
                assert!(
                    set_reverse_lookup_event_v1_num_emitted == 1,
                    set_reverse_lookup_event_v1_num_emitted
                );
    ...
    public fun clear_name_address(
        user: &signer, subdomain_name: Option<String>, domain_name: String
    ) {
        let user_addr = signer::address_of(user);
        let register_name_event_v1_event_count_before =
            domains::get_register_name_event_v1_count();
        ...
        if (option::is_some(&maybe_reverse_lookup_before)) {
            ...
            if (*reverse_lookup_before
                == domains::create_name_record_key_v1(subdomain_name, domain_name)) {
