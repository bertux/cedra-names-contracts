module cedra_names::domains {
    ...
    use cedra_names::price_model;
    use cedra_names::time_helper;
    use cedra_names::token_helper;
    use cedra_names::utf8_utils;
    use cedra_names::verify;
    use cedra_std::table::{Self, Table};
    use cedra_token::property_map::Self;
    ...
    fun init_module(account: &signer) {
        ...
        move_to(
            account,
            NameRegistryV1 { registry: table::new() }
        );

        move_to(
            account,
            SetNameAddressEventsV1 {
        ...
        move_to(
            account,
            RegisterNameEventsV1 {
                register_name_events: account::new_event_handle<RegisterNameEventV1>(
                    account
                )
            }
        );
    ...
    public entry fun init_reverse_lookup_registry_v1(account: &signer) {
        assert!(
            signer::address_of(account) == @cedra_names,
            error::permission_denied(ENOT_AUTHORIZED)
        );

        if (!exists<ReverseLookupRegistryV1>(@cedra_names)) {
            move_to(account, ReverseLookupRegistryV1 { registry: table::new() });

            move_to(
                account,
                SetReverseLookupEventsV1 {
    ...
    fun register_domain_generic(
        sign: &signer, domain_name: String, num_years: u8
        ...
        let registration_duration_secs: u64 =
            time_helper::years_to_seconds((num_years as u64));
        ...
        assert!(
            length <= config::max_domain_length(),
            error::out_of_range(EDOMAIN_TOO_LONG)
        );
        assert!(
            length >= config::min_domain_length(),
            error::out_of_range(EDOMAIN_TOO_SHORT)
        );

        let price = price_model::price_for_domain_v1(length, num_years);
        coin::transfer<CedraCoin>(sign, config::fund_destination_address(), price);
    ...
    public entry fun register_domain(
        ...
        assert!(
            config::is_enabled_for_nonadmin(sign), error::unavailable(ENOT_ENABLED)
        );
        assert!(
            config::unrestricted_mint_enabled(),
            error::permission_denied(EVALID_SIGNATURE_REQUIRED)
        );
    ...
    public entry fun register_domain_with_signature(
        ...
        verify::assert_register_domain_signature_verifies(
            signature, account_address, domain_name
        );
        register_domain_generic(sign, domain_name, num_years);
    }
    ...
    public entry fun register_subdomain(
        ...
        assert!(
            length <= config::max_domain_length(),
            error::out_of_range(ESUBDOMAIN_TOO_LONG)
        );
        assert!(
            length >= config::min_domain_length(),
            error::out_of_range(ESUBDOMAIN_TOO_SHORT)
        );
    ...
    fun register_name_internal(
        ...
        let cedra_names = borrow_global_mut<NameRegistryV1>(@cedra_names);
        ...
        if (option::is_some(&subdomain_name)) {
            let domain_name_record_key =
                create_name_record_key_v1(option::none(), domain_name);
            let (_property_version, domain_expiration_time_sec, _target_address) =
                get_name_record_v1_props(
                    table::borrow(&cedra_names.registry, domain_name_record_key)
                );
            assert!(
        ...
        let tokendata_id =
            token_helper::ensure_token_data(
                subdomain_name, domain_name, config::domain_type()
            );
        let token_id = token_helper::create_token(tokendata_id);

        let (property_keys, property_values, property_types) =
            get_name_property_map(subdomain_name, name_expiration_time_secs);
        ...
        let (_creator, _collection, _name, property_version) =
            token::get_token_id_fields(&token_id);
        let name_record_key = create_name_record_key_v1(subdomain_name, domain_name);
        let name_record =
            create_name_record_v1(
                property_version, name_expiration_time_secs, option::none()
            );

        table::upsert(&mut cedra_names.registry, name_record_key, name_record);
        ...
        if (option::is_none(&reverse_lookup_result)) {
            ...
            set_name_address_internal(
                subdomain_name, domain_name, signer::address_of(sign)
            );
        };

        event::emit_event<RegisterNameEventV1>(
            &mut borrow_global_mut<RegisterNameEventsV1>(@cedra_names).register_name_events,
            RegisterNameEventV1 {
                subdomain_name,
                domain_name,
    ...
    public entry fun force_set_domain_address(
        sign: &signer, domain_name: String, new_owner: address
    ...
    public entry fun force_create_or_seize_domain_name(
        sign: &signer, domain_name: String, registration_duration_secs: u64
    ) acquires NameRegistryV1, RegisterNameEventsV1, ReverseLookupRegistryV1, SetNameAddressEventsV1, SetReverseLookupEventsV1 {
    ...
    public entry fun force_set_expiration(
        sign: &signer,
        subdomain_name: Option<String>,
        domain_name: String,
        expiration_time_sec: u64
        ...
        config::assert_signer_is_admin(sign);
        assert!(
            name_is_registered(subdomain_name, domain_name),
            error::not_found(ENAME_NOT_EXIST)
        );
        let name_record_key = create_name_record_key_v1(subdomain_name, domain_name);
        let cedra_names = borrow_global_mut<NameRegistryV1>(@cedra_names);
        let name_record = table::borrow_mut(&mut cedra_names.registry, name_record_key);
        name_record.expiration_time_sec = expiration_time_sec;
    }
    ...
    public entry fun force_clear_registration(
        ...
        let name_record_key = create_name_record_key_v1(subdomain_name, domain_name);
        let cedra_names = borrow_global_mut<NameRegistryV1>(@cedra_names);
        if (table::contains(&cedra_names.registry, name_record_key)) {
            table::remove(&mut cedra_names.registry, name_record_key);
        };
    }

    /// Clears the user's reverse lookup.
    public fun force_clear_reverse_lookup(
        admin: &signer, account_addr: address
    ...
    public fun name_is_registerable(
        ...
        if (option::is_some(&subdomain_name)
            && name_is_registerable(option::none(), domain_name)) {
            return false
        };
        ...
        let cedra_names = borrow_global<NameRegistryV1>(@cedra_names);
        let name_record_key = create_name_record_key_v1(subdomain_name, domain_name);
        let name_record = table::borrow(&cedra_names.registry, name_record_key);
    ...
    public fun name_is_expired(
        ...
        if (!name_is_registered(subdomain_name, domain_name)) { true }
        else {
            let cedra_names = borrow_global<NameRegistryV1>(@cedra_names);
            let name_record_key = create_name_record_key_v1(subdomain_name, domain_name);
            let name_record = table::borrow(&cedra_names.registry, name_record_key);
    ...
    public fun name_is_expired_past_grace(
        subdomain_name: Option<String>, domain_name: String
    ): bool acquires NameRegistryV1 {
        if (!name_is_registered(subdomain_name, domain_name)) { true }
        else {
    ...
    public fun name_is_registered(
        ...
        let cedra_names = borrow_global<NameRegistryV1>(@cedra_names);
        let name_record_key = create_name_record_key_v1(subdomain_name, domain_name);
        table::contains(&cedra_names.registry, name_record_key)
    }
    ...
    public fun get_name_record_v1(
        ...
        let cedra_names = borrow_global<NameRegistryV1>(@cedra_names);
        let name_record_key = create_name_record_key_v1(subdomain_name, domain_name);
        *table::borrow(&cedra_names.registry, name_record_key)
    }
    ...
    public fun get_name_record_v1_props_for_name(
        ...
        let cedra_names = borrow_global<NameRegistryV1>(@cedra_names);
        let name_record_key = create_name_record_key_v1(subdomain_name, domain_name);
        get_name_record_v1_props(table::borrow(&cedra_names.registry, name_record_key))
    }
    ...
    public fun is_token_owner(
        owner_address: address, subdomain_name: Option<String>, domain_name: String
    ): (bool, TokenId) {
        let token_data_id =
            token_helper::build_tokendata_id(
                token_helper::get_token_signer_address(),
                subdomain_name,
                domain_name
            );
    ...
    public fun name_resolved_address(
        ...
        if (table::contains(&cedra_names.registry, name_record_key)) {
            let name_record = table::borrow(&cedra_names.registry, name_record_key);
            let (_property_version, _expiration_time_sec, target_address) =
                get_name_record_v1_props(name_record);
            target_address
        } else {
            option::none<address>()
    ...
    public entry fun set_domain_address(
        sign: &signer, domain_name: String, new_address: address
    ) acquires NameRegistryV1, ReverseLookupRegistryV1, SetNameAddressEventsV1, SetReverseLookupEventsV1 {
        assert!(
            config::is_enabled_for_nonadmin(sign), error::unavailable(ENOT_ENABLED)
        );
    ...
    public fun set_name_address(
        ...
        assert!(is_owner, error::permission_denied(ENOT_OWNER_OF_NAME));
        let (is_owner, token_id) = is_token_owner(
            signer_addr, subdomain_name, domain_name
        );
        assert!(is_owner, error::permission_denied(ENOT_OWNER_OF_NAME));
        assert!(
            !name_is_expired(subdomain_name, domain_name),
            error::invalid_state(ENAME_EXPIRED)
        );
        ...
        let (_property_version, expiration_time_sec, _target_address) =
            get_name_record_v1_props(&name_record);
        let (property_keys, property_values, property_types) =
            get_name_property_map(subdomain_name, expiration_time_sec);
        token_helper::set_token_props(
            signer_addr,
            property_keys,
            property_values,
            property_types,
    ...
    public entry fun clear_domain_address(
        sign: &signer, domain_name: String
    ) acquires NameRegistryV1, ReverseLookupRegistryV1, SetNameAddressEventsV1, SetReverseLookupEventsV1 {
        assert!(
            config::is_enabled_for_nonadmin(sign), error::unavailable(ENOT_ENABLED)
        );
    ...
    public entry fun clear_subdomain_address(
        sign: &signer, subdomain_name: String, domain_name: String
    ) acquires NameRegistryV1, ReverseLookupRegistryV1, SetNameAddressEventsV1, SetReverseLookupEventsV1 {
        assert!(
            config::is_enabled_for_nonadmin(sign), error::unavailable(ENOT_ENABLED)
        );
    ...
    public fun clear_name_address(
        ...
        let is_name_resolved_address =
            name_resolved_address(subdomain_name, domain_name)
                == option::some<address>(signer_addr);

        assert!(
            (is_owner && !name_is_expired(subdomain_name, domain_name))
                || is_name_resolved_address,
            error::permission_denied(ENOT_AUTHORIZED)
        );
    ...
    public entry fun set_reverse_lookup_entry(
        account: &signer, subdomain_name: String, domain_name: String
    ) acquires NameRegistryV1, ReverseLookupRegistryV1, SetNameAddressEventsV1, SetReverseLookupEventsV1 {
        assert!(
            config::is_enabled_for_nonadmin(account),
            error::unavailable(ENOT_ENABLED)
        );
    ...
    public fun set_reverse_lookup(
        account: &signer, key: &NameRecordKeyV1
    ) acquires NameRegistryV1, ReverseLookupRegistryV1, SetNameAddressEventsV1, SetReverseLookupEventsV1 {
        assert!(
            config::is_enabled_for_nonadmin(account),
            error::unavailable(ENOT_ENABLED)
    ...
    ): Option<NameRecordKeyV1> acquires ReverseLookupRegistryV1 {
        assert!(
            exists<ReverseLookupRegistryV1>(@cedra_names),
            error::invalid_state(EREVERSE_LOOKUP_NOT_INITIALIZED)
        );
        let registry = &borrow_global_mut<ReverseLookupRegistryV1>(@cedra_names).registry;
    ...
    fun set_reverse_lookup_internal(
        ...
        let account_addr = signer::address_of(account);
        let (maybe_subdomain_name, domain_name) = get_name_record_key_v1_props(key);
        let (is_owner, _) = is_token_owner(
            account_addr, maybe_subdomain_name, domain_name
        );
        assert!(is_owner, error::permission_denied(ENOT_AUTHORIZED));
        assert!(
            !name_is_expired(maybe_subdomain_name, domain_name),
            error::invalid_state(ENAME_EXPIRED)
        );
        assert!(
            exists<ReverseLookupRegistryV1>(@cedra_names),
        ...
        let registry =
            &mut borrow_global_mut<ReverseLookupRegistryV1>(@cedra_names).registry;
    ...
    fun clear_reverse_lookup_internal(
        ...
        if (option::is_none(&maybe_reverse_lookup)) { return };
        let NameRecordKeyV1 { subdomain_name, domain_name } =
            option::borrow(&maybe_reverse_lookup);
        assert!(
            exists<ReverseLookupRegistryV1>(@cedra_names),
            error::invalid_state(EREVERSE_LOOKUP_NOT_INITIALIZED)
        );
        ...
        emit_set_reverse_lookup_event_v1(*subdomain_name, *domain_name, option::none());
    }
    ...
    fun clear_reverse_lookup_for_name(
        ...
        if (option::is_some(&maybe_target_address)) {
            ...
            if (option::is_some(&maybe_reverse_lookup)
                && NameRecordKeyV1 { subdomain_name, domain_name }
                    == *option::borrow(&maybe_reverse_lookup)) {
    ...
    fun emit_set_name_address_event_v1(
        ...
        event::emit_event<SetNameAddressEventV1>(
            &mut borrow_global_mut<SetNameAddressEventsV1>(@cedra_names).set_name_events,
            event
        );
    }
    ...
    fun emit_set_reverse_lookup_event_v1(
        ...
        let event = SetReverseLookupEventV1 { subdomain_name, domain_name, target_address };
        ...
        event::emit_event<SetReverseLookupEventV1>(
            &mut borrow_global_mut<SetReverseLookupEventsV1>(@cedra_names).set_reverse_lookup_events,
            event
        );
    }

    public fun get_name_property_map(
        subdomain_name: Option<String>, expiration_time_sec: u64
        ...
        let expiration_time_sec =
            property_map::create_property_value(&expiration_time_sec);
    ...
    public fun create_name_record_v1(
        property_version: u64, expiration_time_sec: u64, target_address: Option<address>
    ): NameRecordV1 {
        NameRecordV1 { property_version, expiration_time_sec, target_address }
    }

    public fun get_name_record_v1_props(name_record: &NameRecordV1):
        (u64, u64, Option<address>) {
        (
            name_record.property_version,
            name_record.expiration_time_sec,
    ...
    public fun get_name_record_key_v1_props(
        name_record_key: &NameRecordKeyV1
    ...
    public fun get_set_name_address_event_v1_count(): u64 acquires SetNameAddressEventsV1 {
        event::counter(
            &borrow_global<SetNameAddressEventsV1>(@cedra_names).set_name_events
        )
    }
    ...
    public fun get_register_name_event_v1_count(): u64 acquires RegisterNameEventsV1 {
        event::counter(
            &borrow_global<RegisterNameEventsV1>(@cedra_names).register_name_events
        )
    }
    ...
    public fun get_set_reverse_lookup_event_v1_count(): u64 acquires SetReverseLookupEventsV1 {
        ...
        event::counter(
            &borrow_global<SetReverseLookupEventsV1>(@cedra_names).set_reverse_lookup_events
        )
    }
