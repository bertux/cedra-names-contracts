#[test_only]
module cedra_names::is_enabled_tests {
    use std::signer;
    use std::string;
    ...
    #[expected_failure(abort_code = 851969, location = cedra_names::domains)]
    ...
    fun is_enabled_set_domain_address_test(
        ...
        // Should not be able to set address because CNS write is disabled
        domains::set_domain_address(
            user, test_helper::domain_name(), signer::address_of(user2)
        )
    }
    ...
    fun is_enabled_clear_domain_address_test(
        ...
        domains::register_domain(user, test_helper::domain_name(), 1);
        domains::set_domain_address(
            user, test_helper::domain_name(), signer::address_of(user)
        );

        // Disable CNS write
        config::set_is_enabled(myself, false);

        // Should not be able to clear address because CNS write is disabled
    ...
    fun is_enabled_clear_subdomain_address_test(
        ...
        domains::clear_subdomain_address(
            user, test_helper::subdomain_name(), test_helper::domain_name()
        )
    }
    ...
    fun is_enabled_set_primary_name_test(
        ...
        // Register the domain succeeds
        domains::register_domain(user, test_helper::domain_name(), 1);
        domains::set_reverse_lookup_entry(
            user, string::utf8(b""), test_helper::domain_name()
        );

        // Disable CNS write
        config::set_is_enabled(myself, false);

        // Should not be able to set primary name because CNS write is disabled
        domains::set_reverse_lookup_entry(
            user, string::utf8(b""), test_helper::domain_name()
        );
    }
    ...
    fun is_enabled_clear_primary_name_test(
        ...
        // Should not be able to clear primary name because CNS write is disabled
