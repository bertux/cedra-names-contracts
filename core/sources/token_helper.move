module cedra_names::token_helper {
    ...
    const DOMAIN_SUFFIX: vector<u8> = b".ced";
    ...
    public fun get_token_signer_address(): address acquires CollectionCapabilityV1 {
        account::get_signer_capability_address(
            &borrow_global<CollectionCapabilityV1>(@cedra_names).capability
        )
    }
    ...
    public(friend) fun initialize(framework: &signer) {
        ...
        let registry_seed =
            utf8_utils::u128_to_string((timestamp::now_microseconds() as u128));
        string::append(&mut registry_seed, string::utf8(b"registry_seed"));
        let (token_resource, token_signer_cap) =
            account::create_resource_account(framework, *string::bytes(&registry_seed));

        move_to(framework, CollectionCapabilityV1 { capability: token_signer_cap });

        // Set up NFT collection
        let description = string::utf8(b".ced names from Cedra Labs");
        let collection_uri = string::utf8(b"https://cedranames.me");
    ...
    public fun get_fully_qualified_domain_name(
        ...
        let subdomain_is_allowed =
            if (option::is_some(&subdomain_name)) {
                let (subdomain_is_allowed, _length) =
                    utf8_utils::string_is_allowed(option::borrow(&subdomain_name));
    ...
    public fun build_tokendata_id(
        ...
        let fq_domain_name = get_fully_qualified_domain_name(
            subdomain_name, domain_name
        );
        token::create_token_data_id(
            token_resource_address, collection_name, fq_domain_name
        )
    }

    public fun latest_token_id(token_data_id: &TokenDataId): TokenId {
        let (creator, _collection_name, _token_name) =
            token::get_token_data_id_fields(token_data_id);
        let largest_tokendata_property_version =
            token::get_tokendata_largest_property_version(creator, *token_data_id);
    ...
    /// Used for building fully qualified domain names (Ex: `{subdomain_name}.{domain_name}.ced`)
    /// If there is no subdomain, just returns the domain name
    public fun combine_sub_and_domain_str(
        subdomain_name: Option<String>, domain_name: String
    ): String {
    ...
    public(friend) fun ensure_token_data(
        subdomain_name: Option<String>, domain_name: String, type: String
    ): TokenDataId acquires CollectionCapabilityV1 {
        let token_resource = &get_token_signer();

        let token_data_id =
            build_tokendata_id(
                signer::address_of(token_resource), subdomain_name, domain_name
            );
    ...
    fun create_token_data(
        ...
        assert!(
            token::check_collection_exists(
                signer::address_of(token_resource), collection_name
            ),
        ...
        let royalty_payee_address: address = @cedra_names;
    ...
    ): TokenId acquires CollectionCapabilityV1 {
        let token_resource = get_token_signer();

        // At this point, property_version is 0
        let (_creator, collection_name, _name) =
            token::get_token_data_id_fields(&tokendata_id);
        assert!(
            token::check_collection_exists(
                signer::address_of(&token_resource), collection_name
            ),
    ...
    public(friend) fun transfer_token_to(
    ...
    fun test_get_fully_qualified_domain_name() {
        assert!(
            get_fully_qualified_domain_name(option::none(), string::utf8(b"test"))
                == string::utf8(b"test.ced"),
            1
        );
        assert!(
            get_fully_qualified_domain_name(
                option::none(), string::utf8(b"wowthisislong")
            ) == string::utf8(b"wowthisislong.ced"),
            2
        );
        assert!(
            get_fully_qualified_domain_name(option::none(), string::utf8(b"123"))
                == string::utf8(b"123.ced"),
            2
        );
        assert!(
            get_fully_qualified_domain_name(
                option::some(string::utf8(b"sub")),
                string::utf8(b"test")
