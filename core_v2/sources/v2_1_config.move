...
module cedra_names_v2_1::v2_1_config {
    ...
    const CONFIG_OBJECT_SEED: vector<u8> = b"CNS v2 config";

    const DOMAIN_COLLECTION_NAME: vector<u8> = b"Cedra Domain Names V2";
    const SUBDOMAIN_COLLECTION_NAME: vector<u8> = b"Cedra Subdomain Names V2";
        ...
        move_to(
            ...
            Config {
                ...
                tokendata_description: string::utf8(
                    b"This is an official Cedra Labs Name Service Name"
                ),
                tokendata_url_prefix: string::utf8(
                    b"https://www.cedranames.me/api/testnet/v2/metadata/"
                ),
    ...
    public fun is_enabled(): bool acquires Config {
        borrow_global<Config>(@cedra_names_v2_1).enabled
    }

    #[view]
    public fun fund_destination_address(): address acquires Config {
        borrow_global<Config>(@cedra_names_v2_1).fund_destination_address
    }

    #[view]
    public fun admin_address(): address acquires Config {
        borrow_global<Config>(@cedra_names_v2_1).admin_address
    }

    #[view]
    public fun max_number_of_seconds_registered(): u64 acquires Config {
        borrow_global<Config>(@cedra_names_v2_1).max_number_of_seconds_registered
    }

    #[view]
    public fun max_domain_length(): u64 acquires Config {
        borrow_global<Config>(@cedra_names_v2_1).max_domain_length
    }

    #[view]
    public fun min_domain_length(): u64 acquires Config {
        borrow_global<Config>(@cedra_names_v2_1).min_domain_length
    }

    #[view]
    public fun tokendata_description(): String acquires Config {
        borrow_global<Config>(@cedra_names_v2_1).tokendata_description
    }

    #[view]
    public fun tokendata_url_prefix(): String acquires Config {
        borrow_global<Config>(@cedra_names_v2_1).tokendata_url_prefix
    }
    ...
    public fun domain_price_for_length(domain_length: u64): u64 acquires Config {
        ...
        if (domain_length == 3) {
            borrow_global<Config>(@cedra_names_v2_1).domain_price_length_3
        } else if (domain_length == 4) {
            borrow_global<Config>(@cedra_names_v2_1).domain_price_length_4
        } else if (domain_length == 5) {
            borrow_global<Config>(@cedra_names_v2_1).domain_price_length_5
        } else {
            borrow_global<Config>(@cedra_names_v2_1).domain_price_length_6_and_above
        }
    }

    #[view]
    public fun subdomain_price(): u64 acquires Config {
        borrow_global<Config>(@cedra_names_v2_1).subdomain_price
    }

    #[view]
    public fun reregistration_grace_sec(): u64 acquires Config {
        borrow_global<Config>(@cedra_names_v2_1).reregistration_grace_sec
    }
    ...
    public fun signer_is_admin(sign: &signer): bool acquires Config {
        signer::address_of(sign) == admin_address()
            || signer::address_of(sign) == @cedra_names_v2_1
    }
    ...
    public entry fun set_is_enabled(sign: &signer, enabled: bool) acquires Config {
        ...
        borrow_global_mut<Config>(@cedra_names_v2_1).enabled = enabled
    }
    ...
    public entry fun set_admin_address(sign: &signer, addr: address) acquires Config {
        ...
        borrow_global_mut<Config>(@cedra_names_v2_1).admin_address = addr
    }
    ...
    public entry fun set_max_number_of_seconds_registered(
        ...
        borrow_global_mut<Config>(@cedra_names_v2_1).max_number_of_seconds_registered =
            max_seconds_registered
    }
    ...
    public entry fun set_max_domain_length(
        ...
        borrow_global_mut<Config>(@cedra_names_v2_1).max_domain_length = domain_length
    }
    ...
    public entry fun set_min_domain_length(
        ...
        assert_signer_is_admin(sign);
        assert!(domain_length > 0, error::invalid_argument(EINVALID_VALUE));
        borrow_global_mut<Config>(@cedra_names_v2_1).min_domain_length = domain_length
    }
    ...
    public entry fun set_tokendata_description(
        ...
        assert_signer_is_admin(sign);
        borrow_global_mut<Config>(@cedra_names_v2_1).tokendata_description = description
    }
    ...
    public entry fun set_tokendata_url_prefix(
        ...
        assert_signer_is_admin(sign);
        borrow_global_mut<Config>(@cedra_names_v2_1).tokendata_url_prefix = url_prefix
    }

    public entry fun set_subdomain_price(sign: &signer, price: u64) acquires Config {
        assert_signer_is_admin(sign);
        borrow_global_mut<Config>(@cedra_names_v2_1).subdomain_price = price
    }
    ...
    public entry fun set_domain_price_for_length(
        ...
        if (length == 3) {
            borrow_global_mut<Config>(@cedra_names_v2_1).domain_price_length_3 = price
        } else if (length == 4) {
            borrow_global_mut<Config>(@cedra_names_v2_1).domain_price_length_4 = price
        } else if (length == 5) {
            borrow_global_mut<Config>(@cedra_names_v2_1).domain_price_length_5 = price
        } else {
            borrow_global_mut<Config>(@cedra_names_v2_1).domain_price_length_6_and_above =
                price
        }
    }
    ...
    public entry fun set_reregistration_grace_sec(
        ...
        assert_signer_is_admin(sign);
        borrow_global_mut<Config>(@cedra_names_v2_1).reregistration_grace_sec =
    ...
    #[test_only]
    friend cedra_names_v2_1::v2_1_price_model;
    #[test_only]
    ...
    #[test_only]
    public fun initialize_cedracoin_for(deployer: &signer) {
        let (burn_cap, mint_cap) =
            cedra_framework::cedra_coin::initialize_for_test(deployer);
    ...
    public fun initialize_for_test(
        cedra_names_v2_1: &signer, cedra: &signer
    ) acquires Config {
        timestamp::set_time_has_started_for_testing(cedra);
        initialize_cedracoin_for(cedra);
        initialize_config(cedra_names_v2_1, @cedra_names_v2_1, @cedra_names_v2_1);
        set_admin_address_test_only(@cedra_names_v2_1);
    }

    #[test(myself = @cedra_names_v2_1)]
    fun test_default_token_configs_are_set(myself: signer) acquires Config {
        ...
        initialize_config(&myself, @cedra_names_v2_1, @cedra_names_v2_1);
        set_fund_destination_address_test_only(@cedra_names_v2_1);
    ...
    #[test(myself = @cedra_names_v2_1, rando = @0x266f, cedra = @0x1)]
    fun test_configs_are_set(
        ...
        assert!(
            max_number_of_seconds_registered() == SECONDS_PER_YEAR * 5,
            4
        );
    ...
    #[test(myself = @cedra_names_v2_1, rando = @0x266f, cedra = @0x1)]
    #[expected_failure(abort_code = 100, location = cedra_names_v2_1::v2_1_config)]
    ...
    #[test(myself = @cedra_names_v2_1, rando = @0x266f, cedra = @0x1)]
    #[expected_failure(abort_code = 327681, location = cedra_names_v2_1::v2_1_config)]
