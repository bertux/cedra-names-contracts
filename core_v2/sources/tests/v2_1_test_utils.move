module cedra_names_v2_1::v2_1_test_utils {
    ...
    public fun print_actual_expected<T: drop>(
        label: vector<u8>, actual: T, expected: T, always: bool
    ) {
        if (!always && &actual == &expected) { return };
        let expected_actual = ActualExpectedDebug { actual, expected };
        print_trace(label, expected_actual);
    }

    public fun print<T: drop>(label: vector<u8>, value: T) {
        let print_debug = PrintDebug { label: string::utf8(label), value };
