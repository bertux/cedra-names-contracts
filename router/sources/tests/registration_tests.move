...
module router::registration_tests {
    ...
    fun test_register_domain(
        ...
        assert!(
            router::is_name_owner(user_addr, domain_name1, option::none()),
            1
        );
    ...
    fun test_register_domain_with_target_addr_and_to_addr(
        ...
        // Register with v1
        cedra_token::token::opt_in_direct_transfer(user2, true);
        router::register_domain(
            user1,
            domain_name1,
            SECONDS_PER_YEAR,
        ...
        assert!(
            router::is_name_owner(user2_addr, domain_name1, option::none()),
            1
        );
        assert!(
            *option::borrow(&router::get_target_addr(domain_name1, option::none()))
                == user2_addr,
            2
        );
        {
            // Primary name should be unset for user1 now that `target_addr` has been changed.
            let (primary_subdomain, primary_domain) =
                router::get_primary_name(user1_addr);
            assert!(primary_subdomain == option::none(), 3);
        ...
        assert!(
            router::is_name_owner(user2_addr, domain_name2, option::none()),
            1
        );
        assert!(
            *option::borrow(&router::get_target_addr(domain_name2, option::none()))
                == user2_addr,
            2
        );
        {
    ...
    fun test_register_subdomain(
        ...
        assert!(
            router::is_name_owner(
                user_addr, domain_name1, option::some(subdomain_name1)
            ),
            1
        );
        ...
        assert!(
            router::is_name_owner(
                user_addr, domain_name2, option::some(subdomain_name2)
            ),
            2
        );
        assert!(
            router::get_subdomain_expiration_policy(domain_name2, subdomain_name2) == 0,
            3
        );
        ...
        assert!(
            router::get_subdomain_expiration_policy(domain_name2, subdomain_name3) == 1,
            3
        );
    }
    ...
    fun test_register_subdomain_with_target_addr_and_to_addr(
        ...
        assert!(
            router::is_name_owner(user2_addr, domain_name1, subdomain_name_opt),
            1
        );
        assert!(
            *option::borrow(&router::get_target_addr(domain_name1, subdomain_name_opt))
                == user2_addr,
            2
        );
        {
        ...
        assert!(
            router::is_name_owner(user2_addr, domain_name2, subdomain_name_opt),
            1
        );
        assert!(
            *option::borrow(&router::get_target_addr(domain_name2, subdomain_name_opt))
                == user2_addr,
            2
        );
        {
