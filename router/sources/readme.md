# Cedra Naming Service

The Cedra Naming Service is a decentralized naming service for the Cedra blockchain.
...
you would send it to `max.ced` instead.
...
- **Glyph**: A single character in a character set. For example, `a` is a single glyph in the `latin` character set,
  ...
  as `simplified_chinese`, or `emojis`, such as this unicorn: 🦄. Cedra is a unicorn-friendly zone. Neigh!
...
As an `NFT`, following our [Digital Asset Standard](https://cedra.dev/standards/digital-asset). When a name is registered, an `NameRecord` object is created with a reference to represent the ownership. This NFT can be
...
This fee is sent to an account controlled by the `Cedra Foundation`, and will be used to fund grants for the ecosystem.
